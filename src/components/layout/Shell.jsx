...
const Shell = ({ children, activeSection, setActiveSection, tasks, events, categories, addTask, updateTask, deleteTask, addCategory, selectedFolder, onSelectFolder, user, onLogout, settings, updateSettings }) => {
  ...
  return (
    <div className="flex h-screen w-full bg-[#f8fafc]">
              ...
              >
                ...
                {React.Children.map(children, child => {
                   if (React.isValidElement(child)) {
                     return React.cloneElement(child, { onOpenTask: openTaskDetail });
                   }
                   return child;
                })}
              </motion.div>
      ...
      <div className="hidden md:flex flex-col flex-1 h-screen overflow-hidden">
        <header className="h-16 border-b border-gray-200 bg-white flex items-center justify-between px-8 z-10 shadow-sm flex-shrink-0">
          ...
          <div className="flex items-center gap-4 relative">
             ...
             >
                <div className="text-right hidden sm:block">
                  <p className="text-xs font-black text-slate-900 leading-none group-hover:text-blue-600 transition-colors">{user ? (user.displayName || 'FocusFlow User') : 'Guest Account'}</p>
